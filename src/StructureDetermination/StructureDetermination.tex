...
\section{Structure Determination based on a Powder Pattern}\label{\chabbr:sec:SDpowder}
...
\section{Single Crystal Diffractometers}\label{\chabbr:sec:SCDiff}


\section{Elements of Rietveld Refinement}\label{\chabbr:sec:Rietveld}
...
% verified against ITC vol. A, page 16
...
10 & \spgr{10} & 83 & \spgr{83}     & 156 & \spgr{162} & 202 & \spgr{202} \\
12 & \spgr{12} & 87 & \spgr{87}     & 162 & \spgr{164} & 204 & \spgr{204} \\
...
\begin{figure}[h!]
\centering\leavevmode
\includegraphics[width=0.8\textwidth]{\graphicspath Pat1D.pdf}
\caption{(a) Plot of a normal distribution mean $\mu=0$, standard deviation $\sigma=1/3$, and weight $Z=13$; (b) autocorrelation $P(u)$ of the profile in (a).  In (c), a second normal distribution is added, which leads to an extra peak at $u=-4$ in (d).}
\label{\chabbr:fig:Pat1D}
\end{figure}




When we add an atom with a different atomic number, $Z_2=4$, to the unit cell at $\mu_2=4$ with $\sigma_2=1/4$ (Fig.~\ref{\chabbr:fig:Pat1D}(c)), then the corresponding Patterson function (Fig.~\ref{\chabbr:fig:Pat1D}(d)) will show an additional maximum due to the presence of inversion symmetry (which is not present in the unit cell itself). The result is that there are now $3$ peaks in the Patterson unit cell. This is readily shown using an explicit computation; if we consider the input function to be given by:
\[
	g(x) = f(x;0,1/3,13) + f(x;4,1/4,4)\ ,
\]
then the autocorrelation is readily shown to be:
\begin{equation*}
\begin{split}
	P(u) &= g(x)\otimes g(-x)\ ;\\
...
One of the important things to note about Fig.~\ref{\chabbr:fig:Pat1D} is that the Patterson function maintains the interatomic distances from the original unit cell. Each pair of peaks indicates an interatomic vector present in the structure; in particular, the smaller peaks in Fig.~\ref{\chabbr:fig:Pat1D}(d) are each at a distance of $4$ units from the central peak, which is the same as the distance between the original peaks in Fig.~\ref{\chabbr:fig:Pat1D}(c).  This is more clearly illustrated using the 2D example in Fig.~\ref{\chabbr:fig:Pat2D}.   
...
\begin{figure}[h!]
\centering\leavevmode
\includegraphics[width=0.8\textwidth]{\graphicspath Pat2D.pdf}
\caption{(a) $2\times 2$ unit cells with three Gaussian peaks in each cell; (b) autocorrelation of (a). Colored vectors indicate a few interatomic vectors.}
\label{\chabbr:fig:Pat2D}
\end{figure}

Fig.~\ref{\chabbr:fig:Pat2D}(a) shows $2\times 2$ unit cells with three randomly placed Gaussian in each cell. In (b), the autocorrelation of (a) is shown; the colored vectors indicate interatomic vectors in (a) that correspond to maxima in the Patterson function in (b). Every interatomic vector in (a) has an equivalent position in (b); note the intensity differences of the peaks in the Patterson function.


\subsection{Harker lines and planes}\label{\chabbr:ssec:Harker}
Crystal symmetry has a systematic effect on the relative locations of peaks in the Patterson function; this was first noticed by D.\ Harker in 1936 \cite{harker1936a}.  Consider the monoclinic space group \spgr{6}, which has a single mirror plane normal to the $\mathbf{b}$ axis. This means that for every atom at position $(x,y,z)$, there is an equivalent atom at position $(x,-y,z)$. Since the Patterson function has peaks corresponding to relative positions of atoms in the unit cell, this means that there will be a peak at:
\[
...
\begin{figure}[h!]
\centering\leavevmode
\includegraphics[width=0.8\textwidth]{\graphicspath flipex1.pdf}
\caption{(a) hypothetical 2D square unit cell;  (b) theoretical charge density of (a); initial charge  density for the charge-flipping algorithm.}
\label{\chabbr:fig:flipex1}
\end{figure}


\subsubsection{2D example}
In this section, we provide a basic numerical example of the algorithm output for a hypothetical 2D crystal structure with three different atomic species in the square unit cell with lattice parameter $a=0.64$ nm: Cu at $(0,0)$ and $(1/2,1/2)$; Al at $(1/2,0)$ and $(0,1/2)$; and Ti at $(0.15,0.20)$, $(0.35,0.80)$, $(0.85,0.70)$, and $(0.65,0.30)$.  This is a primitive structure, so there are no systematic absences. We set $\mathcal{B}$ to contain all reciprocal lattice points with $\sqrt{h^2+k^2}\le 32$, resulting in $3,145$ contributing reflections.  The structure factor moduli $\vert F_{hk}\vert$ as well as the phase factors $\phi_{hk}$ were computed using the standard atomic scattering factors and the elemental Debye-Waller factors.  An initial guess of the charge density is then given by:
\[
	\rho^{(1)}(\mathbf{r})=\frac{1}{A}\sum_{hk\in\mathcal{B}}\vert F_{hk}\vert\mathrm{e}^{-2\pi\mathrm{i}(hx+ky+\phi_{hk}^r)}\ .
\]
Fig.~\ref{\chabbr:fig:flipex1} shows the schematic crystal structure in (a), the correct charge density on a $66\times 66$ pixel grid in (b), and the initial guess of the charge density, $\rho^{(1)}(\mathbf{r})$, with all phases $\phi_{hk}^r$ set to random numbers in the interval $[0,2\pi]$ in (c). Note that both (b) and (c) show $2\times 2$ unit cells.  The parameter $\delta$ is updated in each iteration step to equal $3\%$ of the standard deviation of the reconstructed charge density. The residual $R^{(k)}$ is computed at each step and the iteration is halted when $R^{(k)}$ no longer decreases, averaged over several iteration steps.  Fig.~\ref{\chabbr:fig:flipex2}(a) shows the residual $R^{(k)}$ as a function of iteration number $k$; convergence is obtained after $56$ steps, and the resulting charge density is shown in Fig.~\ref{\chabbr:fig:flipex2}(b). The number of iteration steps is different each run due to the choice of a seed for the random number generator used to create the initial phases. The final phases $\phi_{hk}$ are determined modulo a constant additive factor, which means that the origin of the reconstructed charge density does not necessarily coincide with that of the original structure; furthermore, the algorithm often produces the mirror image of the charge density. Both of these artifacts can be easily corrected.  The charge flipping algorithm can be applied to 3D diffraction data as well as higher dimensional data, e.g., for incommensurate structures and quasi-crystals.  

\begin{figure}[h!]
\centering\leavevmode
\includegraphics[width=0.8\textwidth]{\graphicspath flipex2.pdf}
\caption{(a) plot of the residual $R^{(k)}$ as a function of the iteration number $k$; (b) final charge  density with the original unit cell overlaid. Note that the cell is shifted from the original location.}
\label{\chabbr:fig:flipex2}
\end{figure}
