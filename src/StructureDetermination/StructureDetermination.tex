...
In this section, we provide a basic numerical example of the algorithm output for a hypothetical 2D crystal structure with three different atomic species in the square unit cell with lattice parameter $a=0.64$ nm: Cu at $(0,0)$ and $(1/2,1/2)$; Al at $(1/2,0)$ and $(0,1/2)$; and Ti at $(0.15,0.20)$, $(0.35,0.80)$, $(0.85,0.70)$, and $(0.65,0.30)$.  This is a primitive structure, so there are no systematic absences. We set $\mathcal{B}$ to contain all reciprocal lattice points with $\sqrt{h^2+k^2}\le 16$, resulting in $797$ contributing reflections.  The structure factor moduli $\vert F_{hkl}\vert$ as well as the phase factors $\phi_{hkl}$ were computed using the standard atomic scattering factors.  An initial guess of the charge density is then given by:
\[
	\rho^{(1)}(\mathbf{r})=\frac{1}{A}\sum_{hk\in\mathcal{B}}\vert F_{hk}\vert\mathrm{e}^{-2\pi\mathrm{i}(hx+ky)}\ .
\]
Fig.~\ref{\chabbr:fig:flipex1} shows the schematic crystal structure in (a), the correct charge density on a $66\times 66$ pixel grid in (b), and the initial guess of the charge density, $\rho^{(1)}(\mathbf{r})$, with all phases set to zero in (c). Note that both (b) and (c) show $2\times 2$ unit cells.  The parameter $\delta$ is set to $10\%$ of the maximum of the $\vert G_{hk}\vert$ values, referring to the flow diagram in Fig.~\ref{\chabbr:fig:chargeflip}. The residual $R^{(k)}$ is computed at each step and the iteration is halted when $R^{(k)} < xxx$.  Fig.~\ref{\chabbr:fig:flipex2}(a) shows the residual $R^{(k)}$ as a function of iteration number $k$; convergence is obtained after XXX steps, and the resulting charge density is shown in Fig.~\ref{\chabbr:fig:flipex2}(b).  The phases $\phi_{hkl}$ are determined modulo a constant additive factor, which means that the origin of the reconstructed charge density does not necessarily coincide with that of the original structure.  

% perform two runs, one with initial phases set to 0, the other with randomized phases
