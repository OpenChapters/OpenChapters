...
\chapterimage{GRAINBheader.pdf}

%	Make sure to use the \label{\chabbr:} command everywhere instead of \label{} !
\chapter{Grain Boundaries}\label{\chabbr:ch:GB}

% add the author information so that it will appear in the Author List at the start of the document
\writeauthor{GRAINB:ch:GB}{Grain Boundaries}{De Graef}{Marc}{Materials Science and Engineering}{Carnegie Mellon University}{mdg@andrew.cmu.edu}{https://www.mse.engineering.cmu.edu/directory/bios/degraef-marc.html}
...
{\begin{itemize}
    \item[{\color{OCBurntOrange}\ref{\chabbr:sec:GBgeometry}:}] Learn how to describe a grain boundary
    \item[{\color{OCBurntOrange}\ref{\chabbr:sec:MacKenzie}:}] Determine disorientation distributions
    \item[{\color{OCBurntOrange}\ref{\chabbr:sec:MFZs}:}] Describe misorientation fundamental zones
    \item[{\color{OCBurntOrange}\ref{\chabbr:sec:twophases}:}] Learn about phase boundaries
\end{itemize}}
...
In section~\ref{3DROTS:ssec:misorientations} we introduced the concept of the \indexit{misorientation} between two orientations; a simple example will illustrate the concept. Consider two rotations around the $[100]$ axis, one counterclockwise by $\pi/3$, the other clockwise.  The corresponding rotation quaternions are then given by:
\[
...
\begin{figure}[h]
\centering\leavevmode
\includegraphics[width=4.in]{\graphicspath GBsketch.pdf}
...
\begin{figure}[hb]
\centering\leavevmode
\includegraphics[width=5.in]{\graphicspath tilttwist.pdf}
...
\begin{figure}[t]
\centering\leavevmode
\includegraphics[width=3.in]{\graphicspath CSLgrid.pdf}
...
21a& 21.79&  111&  14.03& 79.02& 75.97&  1/9&  1/9&  1/9& 0.982 & 0.109& 0.109& 0.109\\
...
\begin{figure}[h]
\centering\leavevmode
\includegraphics[width=2.5in]{\graphicspath CSL-tilt.pdf}
...
\begin{align*}
	q_A &= [0.994522, 0.0603495, 0.0603495, 0.0603495];\\
	q_B &= [0.994522,-0.0603495, -0.0603495, -0.0603495].
\end{align*}
The misorientation is then $q^{\ast}_Aq_B = [0.978148, -0.120038, -0.120038, -0.120038]$ or its complex conjugate; note that the grain boundary misorientation angle equals $24^{\circ}$.  To find the closest CSL we need to compute the mis-misorientation for each of the CSL boundaries in Table~\ref{\chabbr:tb:CSL}; we represent the CSL quaternions by $q_k$ with $k\in[1\ldots 28]$.  The computation then requires finding the following minimum over all combinations of $i$, $j$, and $k$:
\[
	\min_{ijk}\left\{2\arccos[q_k\cdot((q_jq_B)^{\ast}(q_iq_A))],2\arccos[q_k\cdot((q_jq_A)^{\ast}(q_iq_B))] \right\}.
\]
This computation represents an easy programming problem, and the result is that the minimum is obtained for the $\Sigma 21a$ CSL, with a misorientation angle of $2.21^{\circ}$. Table~\ref{\chabbr:tb:CSL} shows that $\Sigma 21a$ corresponds to the rotation $21.79^{\circ}@[111]$, whereas the selected boundary has a misorientation angle of $24^{\circ} = 21.79^{\circ}+2.21^{\circ}$. The mis-misorientation angle is smaller than the maximum angle of $15^{\circ}/\sqrt{21}=3.27^{\circ}$ from the Brandon criterion, indicating that the twist boundary between grains $A$ and $B$ is close to a CSL of type $\Sigma 21a$.
...
\begin{figure}[ht]
\centering\leavevmode
\includegraphics[width=4.5in]{\graphicspath twinschematic.pdf}
...
\end{equation}
the function is only evaluated up to the maximum angle $\omega_\text{max}$ for points inside the RFZ, and the factor $N$ guarantees that the integral over the entire Rodrigues space remains equal to $1$.
...
\begin{figure}[t!]
\centering\leavevmode
\includegraphics[width=\textwidth]{\graphicspath MK-cyclic.pdf}
...
\begin{figure}[t!]
\centering\leavevmode
\includegraphics[width=0.5\textwidth]{\graphicspath SphericalCap.pdf}
\caption{\label{\chabbr:fig:SphericalCap}Illustration of the protruding sphere at various points along the curve for point group \pg{12}.}
\end{figure}
...
The same procedure can be carried out for all the crystallographic point groups.  As examples we consider first the cyclic groups and then the orthorhombic crystal system.  The RFZs for the cyclic point groups are represented by an infinite slab of thickness $2\tan(\pi/2n)$ normal to the rotation axis. For \pg{1}, the RFZ is the entire Rodrigues space so that inclusion of the inversion operator in point group \pg{2} simply divides the space in half; one can select any plane through the origin to split the space, but the most convenient choice is to consider as the MFZ for \pg{2} all Rodrigues vectors with $\rho_z\ge 0$. For point group \pg{3}, the monoclinic $\mathbf{b}$ axis is parallel to the rotation axis (the $\rho_y$ axis in Rodrigues space) and there are two additional point groups, \pg{4} and \pg{5}.  For \pg{4}, we select all Rodrigues vectors with $0\le\rho_y\le1$ to form the MFZ; for \pg{5}, we cut the \pg{4} MFZ in half by selecting only Rodrigues vectors with $0\le\rho_y\le1$ \textit{and} $\rho_z\ge 0$. For \pg{17}, we take $0\le\rho_z\le 1/\sqrt{3}$, as it is for \pg{16}, but we add the restriction that $\vert\rho_y\vert\le \rho_x\sqrt{3}$ to obtain a $120^{\circ}$ infinite wedge as the MFZ.  For cyclic group \pg{9}, we have two groups to consider: \pg{10} and \pg{11}. For \pg{10} we select $0\le\rho_z\le\sqrt{2}-1$ \textit{and} $\rho_x\ge 0$, while for \pg{11} the additional restriction $\rho_y\ge 0$ must be added.  Finally, for cyclic group \pg{21} we have two cases to consider: \pg{22} and \pg{23}. For \pg{22} we have $0\le\rho_z\le (2-\sqrt{3})$ and $\vert\rho_y\vert\le\rho_x\sqrt{3}$, whereas for \pg{23} the second condition becomes $0\le\rho_y\le\rho_x\sqrt{3}$.  This concludes the enumeration of the cyclic MFZs.  The $13$ cyclic MFZs are represented as 2-D projections along one of the Rodrigues axes in Fig.~\ref{\chabbr:fig:cyclicMFZs}. The axis range on all plots is from $-1.5$ to $+1.5$ for all axes, similar to the plots in Fig.~\ref{ORISYM:fig:cyclicFZ}.  The green shaded regions in each plot are within the respective MFZ and wherever the green volume touches the exterior frame of the plot, an infinite line from the origin through that point is implied.  The corresponding point group symbol is shown below each plot.

\begin{figure}[t!]
\centering\leavevmode
\includegraphics[width=\textwidth]{\graphicspath cyclicMFZs.pdf}
\caption{\label{\chabbr:fig:cyclicMFZs}Misorientation fundamental zones for all cyclic point groups and the groups derived from them.}
\end{figure}


For the orthorhombic crystal system, we know that the RFZ of \pg{6} is a cube of edge length $2$ centered at the origin of Rodrigues space;  we will need to further subdivide this cube to obtain the MFZs.  If we pick an arbitrary direction $[uvw]$ representing a misrotation axis, then Table~\ref{\chabbr:tb:directions} lists the equivalent axes under the three orthorhombic point group symmetries.

\begin{table}[hb]
\centering\leavevmode
\begin{tabular}{|c|l|}
\hline
Point Group & Equivalent Misorientation Axes\\
\hline\hline
\pg{6} & $[uvw], [\bar{u}\bar{v}w], [\bar{u}v\bar{w}], [u\bar{v}\bar{w}]$\\
\hline
\pg{7} & $[uvw], [\bar{u}\bar{v}w], [u\bar{v}w], [\bar{u}vw]$\\
\hline
\pg{8} & $[uvw], [\bar{u}\bar{v}\bar{w}], [\bar{u}\bar{v}w], [\bar{u}v\bar{w}],  [u\bar{v}\bar{w}], [uv\bar{w}], [u\bar{v}w], [\bar{u}vw]$\\
\hline
\end{tabular}
\caption{\label{\chabbr:tb:directions}Equivalent misorientation axes for the three orthorhombic point group symmetries.}
\end{table}

Fig.~\ref{\chabbr:fig:orthoMFZs}(a) shows the orthorhombic fundamental zone outlined in red (a cube with edge length $2$, centered on the origin of Rodrigues space). From the first line of Table~\ref{\chabbr:tb:directions} we see that the list of misorientation axes equivalent to $[uvw]$ does not contain $[\bar{u}\bar{v}\bar{w}]$, so it appears logical that the MFZ for \pg{6} consists of two unit cubes in opposite octants.  For \pg{7}, all equivalent directions have the same sign for the third index, so the MFZ should also include the unit cube in the lower octant, as shown in Fig.~\ref{\chabbr:fig:orthoMFZs}(b). Finally, for \pg{8} the equivalent misorientation axes belong to each of the octants, so a single octant is sufficient as the MFZ (Fig.~\ref{\chabbr:fig:orthoMFZs}(c)).

\begin{figure}[t!]
\centering\leavevmode
\includegraphics[width=\textwidth]{\graphicspath orthoMFZs.pdf}
\caption{\label{\chabbr:fig:orthoMFZs}Misorientation fundamental zones for the orthorhombic point groups.}
\end{figure}

Using a similar approach for the $16$ remaining point groups we can derive their MFZs; they are shown in Fig.~\ref{\chabbr:fig:MFZs}.  As a general rule, the MFZ is sub-divided into $N$ polyhedra, where $N$ is the order of the point group.  The MFZ can thus be regarded as an asymmetric unit that reconstructs the entire RFZ when all $N$ symmetry operators are applied to it.  It is important to appreciate the different roles the operators of the point group play when it comes to the RFZ and the MFZ.  To define the RFZ, we use the rotational symmetry operators of the crystal to determine the indistinguishable orientations of the crystal's unit cell with respect to the external sample reference frame.  This leads, by definition, to a single point inside the RFZ and $N-1$ points outside of it for a given orientation.  For the MFZ, there is no external reference frame, and we use those same symmetry operators to determine the equivalent misorientation axes for a given misorientation, where we ensure that the misorientation angle is limited to the range $0\le\omega\le\omega_\text{max}$.  Whereas the shape of the RFZ is determined, for a given crystal system, by its proper or rotational point groups only, for the MFZ we need to use the full point group symmetry, including any mirror planes and/or the inversion; the RFZ is then sub-divided into $N$ identical polyhedral units where $N$ is the order of the full point group.  For instance, if we were to plot misorientations across grain boundaries in $\alpha$-Ti, which has point group symmetry \pg{27}, then we must use the MFZ for this point group (3rd entry on 3rd row in Fig.~\ref{\chabbr:fig:MFZs}), not the MFZ for the rotational group \pg{24}, since this MFZ would not represent the correct symmetry relations between misorientation axes due to its lack of mirror planes.


\begin{figure}[t!]
\centering\leavevmode
\includegraphics[width=\textwidth]{\graphicspath MFZs.pdf}
\caption{\label{\chabbr:fig:MFZs}Misorientation fundamental zones (in blue) of the remaining $16$ point groups.}
\end{figure}


\subsection{Misorientation Fundamental Zones and CSL Boundaries}\label{\chabbr:ssec:MFZCSL}
In Table~\ref{\chabbr:tb:CSL} we listed $28$ CSL boundaries in the face-centered cubic lattice, and in the previous subsection we described the shape of the octahedral MFZ.  In this section we put these two together and provide a graphical illustration of the CSL points inside the MFZ.  It should be noted from Table~\ref{\chabbr:tb:CSL} that the majority of the rotation axes of the CSLs are special directions in the cubic system, namely the $[100]$, $[110]$ and $[111]$ directions which correspond to four-fold, two-fold, and three-fold rotation axes, respectively.  The axes are also the outer edges of the octahedral MFZ.   In addition, there are five other axes: $[210]$, $[211]$, $[221]$, $[311]$, and $[331]$, each with a few CSL points.  

Working in Rodrigues space, we can represent each CSL boundary by a small colored sphere located at the appropriate distance from the origin along the proper rotation axis.  This is illustrated in Fig.~\ref{\chabbr:fig:MFZCSL}; all the rotation axes are represented as blue cylinders starting in the origin and each sphere is labeled with its CSL symbol, but with the symbol $\Sigma$ omitted.    

\begin{figure}[t!]
\centering\leavevmode
\includegraphics[width=0.75\textwidth]{\graphicspath MFZCSL.pdf}
\caption{\label{\chabbr:fig:MFZCSL}Misorientation fundamental zone for octahedral symmetry with all 28 CSL boundary positions listed in Table~\protect\ref{\chabbr:tb:CSL} superimposed as colored spheres; note that the $\Sigma$ symbol has been omitted from the labels.}
\end{figure}



\section{Boundaries between Different Crystallographic Phases}\label{\chabbr:sec:twophases}



%
%\newcommand{\mini}[1]{\begin{minipage}[c]{0.065in}#1\end{minipage}}
%\begin{landscape}
%\begin{table}
%\caption{Rodrigues misorientation fundamental zone types for all pairs of crystallographic point groups; horizontal and vertical lines delineate the crystal systems. The shapes $a$ through $i$ are illustrated in Figs.~\protect\ref{fig:RFZs} and \protect\ref{fig:DFZs}. }\label{\chabbr:tb:mFZs}
%\begin{tabular}{cc|ccc|ccc|ccccccc|ccccc|ccccccc|ccccc|c}
%  \mini{1}&  \mini{2}&  \mini{3}&  \mini{4}&  \mini{5}&  \mini{6}&  \mini{7}&  \mini{8}&  \mini{9}&\mini{10}&
%\mini{11}&\mini{12}&\mini{13}&\mini{14}&\mini{15}&\mini{16}&\mini{17}&\mini{18}&\mini{19}&\mini{20}&
%\mini{21}&\mini{22}&\mini{23}&\mini{24}&\mini{25}&\mini{26}&\mini{27}&\mini{28}&\mini{29}&\mini{30}&
%\mini{31}&\mini{32}& PG\\
%\hline
%x&x&x&x&x&i&x&i&x&x&x&g&x&i&g&x&x&h&x&h&x&x&x&f&x&h&f&d&d&c&d&c&1\\
%  &x&x&x&x&i&x&i&x&x&x&g&x&i&g&x&x&h&x&h&x&x&x&f&x&h&f&d&d&c&d&c&2\\
%\hline
%  &  &x&x&x&i&i&i&g&i&g&g&g&g&g&h&h&f&h&f&f&h&f&f&f&f&f&d&d&c&d&c&3\\
%  &  &  &x&x&i&x&i&x&x&x&g&x&i&g&x&x&h&x&h&x&x&x&f&x&h&f&d&d&c&d&c&4\\
%  &  &  &  &x&i&i&i&g&i&g&g&g&g&g&h&h&f&h&f&f&h&f&f&f&f&f&d&d&c&d&c&5\\
%\hline
%  &  &  &  &  &i&i&i&g&i&g&g&g&g&g&f&f&f&f&f&f&f&f&f&f&f&f&d&d&c&d&c&6\\
%  &  &  &  &  &  &x&i&x&x&x&g&x&i&g&x&x&f&x&f&x&x&x&f&x&f&f&d&d&c&d&c&7\\
%  &  &  &  &  &  &  &i&g&i&g&g&g&g&g&f&f&f&f&f&f&f&f&f&f&f&f&d&d&c&d&c&8\\
%\hline
%  &  &  &  &  &  &  &  &x&x&x&g&x&g&g&x&x&e&x&e&x&x&x&e&x&e&e&c&c&c&c&c&9\\
%  &  &  &  &  &  &  &  &  &x&x&g&x&i&g&x&x&f&x&f&x&x&x&f&x&f&f&d&d&c&d&c&10\\
%  &  &  &  &  &  &  &  &  &  &x&g&x&g&g&x&x&e&x&e&x&x&x&e&x&e&e&c&c&c&c&c&11\\
%  &  &  &  &  &  &  &  &  &  &  &g&g&g&g&e&e&e&e&e&e&e&e&e&e&e&e&c&c&c&c&c&12\\
%  &  &  &  &  &  &  &  &  &  &  &  &x&g&g&x&x&e&x&e&x&x&x&e&x&e&e&c&c&c&c&c&13\\
%  &  &  &  &  &  &  &  &  &  &  &  &  &i&g&f&f&e&f&e&f&f&f&e&f&e&e&c&c&c&c&c&14\\
%  &  &  &  &  &  &  &  &  &  &  &  &  &  &g&e&e&e&e&e&e&e&e&e&e&e&e&c&c&c&c&c&15\\
%\hline
%  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &x&x&h&x&h&x&x&x&f&x&h&f&b&b&a&b&a&16\\
%  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &x&h&x&h&x&x&x&f&x&h&f&b&b&a&b&a&17\\
%  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &h&x&h&f&h&f&f&f&h&f&b&b&a&b&a&18\\
%  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &x&h&x&x&x&f&x&h&f&b&b&a&b&a&19\\
%  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &h&f&h&f&f&f&h&f&b&b&a&b&a&20\\
%\hline
%  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &x&x&x&f&x&f&f&b&b&a&b&a&21\\
%  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &x&x&f&x&h&f&b&b&a&b&a&22\\
%  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &x&f&x&f&f&b&b&a&b&a&23\\
%  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &f&f&f&f&b&b&a&b&a&24\\
%  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &x&f&f&b&b&a&b&a&25\\
%  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &h&f&b&b&a&b&a&26\\
%  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &f&b&b&a&b&a&27\\
%\hline
