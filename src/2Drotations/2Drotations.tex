...
% All figures are stored in the src/2Drotations/eps folder and must be of the *.pdf type. 
...
\HTMLPageTop{\begin{center}\fbox{\includegraphics{\graphicspath 2DROTSheader.pdf}}\end{center}}
}{}%

\chapter{2D Rotations}\label{\chabbr:ch:2Drotations}

% add the author information so that it will appear in the Author List at the start of the document
\writeauthor{2DROTS:ch:2Drotations}{2D Rotations}{De Graef}{Marc}{Materials Science and Engineering}{Carnegie Mellon University}{mdg@andrew.cmu.edu}{https://www.mse.engineering.cmu.edu/directory/bios/degraef-marc.html}
...
\begin{multicols}{2}
\begin{itemize}
    \item[{\color{OCBurntOrange}\ref{\chabbr:sec:rotationmatrices}:}] Perform 2D passive and active rotations with matrices
    \item[{\color{OCBurntOrange}\ref{\chabbr:sec:complex2D}:}] Use complex numbers for 2D rotations
    \item[{\color{OCBurntOrange}\ref{\chabbr:sec:2Dcomposition}:}] Combine multiple rotations
    \item[{\color{OCBurntOrange}\ref{\chabbr:sec:2Darbitrary}:}] Combine rotations with translations using homogeneous coordinates   
\end{itemize}
\end{multicols}}}
% ###################################################################
% ###################################################################
...
\label{\chabbr:sec:rotationmatrices}
...
\subsection{Active 2D rotations}\label{\chabbr:ssec:2Dactive}
Consider the position vector $\mathbf{r}$ with components $(x,y)$ with respect to the Cartesian basis vectors $\mathbf{e}_1$ and $\mathbf{e}_2$, as shown in Fig.~\ref{\chabbr:fig2Drot}(a).  The vector makes an angle $\theta$ with respect to the $\mathbf{e}_1$ axis and has length $r$, so that the coordinates can be expressed in term of the polar components as $(x,y) = r(\cos\theta,\sin\theta)$.  We rotate the vector counter-clockwise by an angle $\omega$ to $\mathbf{r}'$, so that the new components are given by $(x',y') = r\left(\cos(\theta+\omega),\sin(\theta+\omega)\right)$.  Using the sum formulas for trigonometric functions we can write:
\begin{align*}
...
\subsection{Passive 2D rotations}\label{\chabbr:ssec:2Dpassive}
For a passive rotation, shown in Fig.~\ref{\chabbr:fig2Drot}(b), we keep the vector $\mathbf{r}$ at a constant orientation in space, and we rotate the reference frame $\mathbf{e}_i$ by a positive angle $\omega$ (i.e., counter-clockwise) to the frame $\mathbf{e}'_i$.  To perform this rotation, we note that the components of the new basis vectors in terms of the old ones can be written in terms of the projections of the new vectors onto the old ones; since these are unit vectors, the projections are simply equivalent to the dot products between the new and old basis vectors, and we have:
\[
...
\]
From the sketch in Fig.~\ref{\chabbr:fig2Drot}(b) we find that the matrix components are given by:
\[
...
\section{Complex number representation}\label{\chabbr:sec:complex2D}
...
\section{Composition of 2D rotations}\label{\chabbr:sec:2Dcomposition}
...
\]	
Since 3D rotations do not commute in general, the generalization of complex numbers to quaternions will need to incorporate this non-commutative behavior; in section~\ref{NUMSYS:sec:quaternions} we will see that the product of two quaternions, each representing a 3D rotation, does not commute.  For active rotations, we write the consecutive rotations from right to left; for passive rotations we reverse the multiplication order.

\section{2D rotations combined with translations}\label{\chabbr:sec:2Darbitrary}
Translations become important in the description of crystal structures and crystal symmetry, so in this section we describe how we can combine a 2D rotation with a translation in terms of a single matrix operation.  Consider an operation consisting of a rotation, $R_{ij}$, followed by a translation by a vector $\mathbf{t}=(t_x,t_y)$ so that the new vector $\mathbf{r}'$ is given by $\mathbf{r}' = R\,\mathbf{r}+\mathbf{t}$ . In component notation we have:
\[
	r'_i = R_{ij}\, r_j + t_i\ .
\]
In the field of mathematics known as \indexit{projective geometry}, one extends the standard Euclidean geometry to include the points at infinity by introducing \indexit{homogeneous coordinates}; this means that a third coordinate is added to the pair $(x,y)$ to become $(wx,wy,w)$ where $w\in\mathbb{R}$. In the special case of $w=0$, the homogeneous coordinates become $(x,y,0)$ and this indicates the point at infinity in the direction of the vector $(x,y)$.  In practice, we put $w=1$ although all values of $w\ne0$ represent the same point $(x,y)$.  The 2D homogeneous coordinates of the point $(x,y)$ are therefore $(x,y,1)$.  Restating the above equation in homogeneous coordinates and combining rotation and translation into a single $3\times 3$ matrix we have:
\[
	\begin{pmatrix}x'\\ y'\\ 1\end{pmatrix} = \begin{pmatrix} R_{xx} & R_{xy} & t_x\\ R_{yx} & R_{yy} & t_y\\ 0&0&1\end{pmatrix} \begin{pmatrix}x\\ y\\ 1\end{pmatrix}\ .
\]
It is easy to see that this relation reproduces the original one, and the third equation simply states that $1=1$.  

We can use this approach to work out what the transformation matrix would be for a counterclockwise rotation by an angle $\theta$ with the rotation axis located at a point $\mathbf{p}=(p_x,p_y)$. In vector notation, this would be equivalent to:
\[
	\mathbf{r}' = R\,(\mathbf{r}-\mathbf{p}) + \mathbf{p}\ ;
\]
in other words, we translate the rotation axis to the origin by means of the vector $-\mathbf{p}$, then we apply the rotation, and then we translate back to $\mathbf{p}$. In matrix notation, using homogeneous coordinates, this results in:
\begin{equation*}
\begin{split}
	\begin{pmatrix}x'\\ y'\\ 1\end{pmatrix} &= 
	\begin{pmatrix} 1 & 0 & p_x\\ 0 & 1 & p_y\\ 0&0&1\end{pmatrix}
	\begin{pmatrix} \cos\theta & -\sin\theta & 0\\ \sin\theta & \cos\theta & 0\\ 0&0&1\end{pmatrix} 
	\begin{pmatrix} 1& 0 & -p_x\\ 0 & 1 & -p_y\\ 0&0&1\end{pmatrix}
	\begin{pmatrix}x\\ y\\ 1\end{pmatrix}\\
	& = 	\begin{pmatrix} \cos\theta & -\sin\theta & p_x(1-\cos\theta)+p_y\sin\theta\\ \sin\theta & \cos\theta & p_y(1-\cos\theta)-p_x\sin\theta\\ 0&0&1\end{pmatrix} 	\begin{pmatrix}x\\ y\\ 1\end{pmatrix}\ .
\end{split}
\end{equation*}
This approach can easily be extended to rotations and translations in 3D.
